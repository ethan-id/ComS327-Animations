...
void attackPlayer(worldPokemon *enemy, WINDOW *win, int enemyMove) {
    ...
    if ((rand() % 100) < enemy->pokeMoves[enemyMove].accuracy) {
        ...
        mvwprintw(win, 5, 11, "%s did %d damage!%30s", enemy->pokeMoves[enemyMove].identifier, (int)enemyDamage, "");
        wrefresh(win);
        usleep(1500000);
    } else {
        mvwprintw(win, 2, 10, "%s missed...%20s", enemy->pokeMoves[enemyMove].identifier, "");
        mvwprintw(win, 5, 11, "%-60s", "");
        usleep(1500000);
    }
}
...
void attackSequence(worldPokemon *enemy, WINDOW *win, move_db moveMade) {
    ...
    if (moveMade.priority > enemyPriority) {
        if ((rand() % 100) < moveMade.accuracy) {
            ...
            mvwprintw(win, 5, 11, "%s did %d damage!%30s", moveMade.identifier, (int)damage, "");
            wrefresh(win);
            usleep(1500000);
        } else {
...
int battleSequence(worldPokemon *enemy, WINDOW *win, int isWild) {
    for (int i = 1; i < 56; i++) {
        mvwprintw(win, 2, 1, "%-70s", "");
        mvwprintw(win, 2, i, "%-22s", enemy->name);
        mvwprintw(win, 3, 1, "%-70s", "");
        mvwprintw(win, 3, i, "HP: %3d / %-3d%-5s", enemy->hp, enemy->initialHP, "");
        wrefresh(win);
        usleep(10000);
    }

    for (int i = 1; i < 67; i++) {
        mvwprintw(win, 9, 10, "%-65s", "");
        mvwprintw(win, 9, 77 - i, "%-22s", heldPokemon[activePokemon].name);
        mvwprintw(win, 10, 10, "%-65s", "");
        mvwprintw(win, 10, 77 - i, "HP: %3d / %-3d%-5s", heldPokemon[activePokemon].hp, heldPokemon[activePokemon].initialHP, "");
        wrefresh(win);
        usleep(10000);
    }

    mvwprintw(win, 7, 25, "You are now battling a %s%20s", enemy->name, "");
    wrefresh(win);
    usleep(375000);
    mvwprintw(win, 7, 25, "%-50s", "");
    wrefresh(win);
    usleep(375000);
    mvwprintw(win, 7, 25, "You are now battling a %s%20s", enemy->name, "");
    wrefresh(win);
    ...
    while (battling) {
        ...
        if (enemy->hp != enemy->initialHP) {
            double health = (((double) enemy->hp) / enemy->initialHP) * 100;
            int numHP = firstDigit((int) health);
            mvwprintw(win, 4, 55, "{%-10d}", numHP);
            for (int i = 0; i < numHP; i++) {
                mvwprintw(win, 4, 56 + i, "O");
            }
        } else {
            mvwprintw(win, 4, 55, "{OOOOOOOOOO)");
        }

        mvwprintw(win, 9, 10, "%-22s", heldPokemon[activePokemon].name);
        mvwprintw(win, 10, 10, "HP: %3d / %-3d%-5s", heldPokemon[activePokemon].hp, heldPokemon[activePokemon].initialHP, "");
        if (heldPokemon[activePokemon].hp != heldPokemon[activePokemon].initialHP) {
            double health2 = (((double) heldPokemon[activePokemon].hp) / heldPokemon[activePokemon].initialHP) * 100;
            int numHP2 = firstDigit((int) health2);
            mvwprintw(win, 11, 10, "{%-10d}", numHP2);
            for (int i = 0; i < numHP2; i++) {
                mvwprintw(win, 11, 11 + i, "O");
            }
        } else {
            mvwprintw(win, 11, 10, "{OOOOOOOOOO}");
        }
        ...
        while(strcmp(str, "") == 0) {
                ...
                while(strcmp(str, "") == 0) {
                    ...
                    if (strcmp(str, "1") == 0) {
                        ...
                        if (revives > 0) {
                            ...
                            mvwprintw(win, 5, 11, "%25s", "You're out of revives.");
                            wrefresh(win);
                            usleep(1500000);
                        }
                    } else if (strcmp(str, "3") == 0) {
                        ...
                        if (!isWild || enemy->hp != 0) {
                            mvwprintw(win, 5, 10, "%25s", "You can't capture this pokemon.");
                            wrefresh(win);
                            usleep(1500000);
                        }
                    } else {
            ...
            // Run
                ...
                if (isWild) {
                    mvwprintw(win, 5, 11, "You ran away...");
                    wrefresh(win);
                    usleep(1000000);
                    battling = 0;
                } else {
                    mvwprintw(win, 5, 11, "You can't run away from a trainer");
                    wrefresh(win);
                    usleep(1000000);
                }
            // Switch Pokemon
                ...
                if (numPokemon > 1) {
                    ...
                    while(strcmp(str, "") == 0) {
                        ...
                        if (heldPokemon[activePokemon].hp <= 0) {                
                            mvwprintw(win, 5, 11, "You can't switch to a pokemon with 0 hp.");
        ...
        noecho();
        if (heldPokemon[activePokemon].hp == 0) {
            mvwprintw(win, 5, 11, "Your %s was defeated...%30s", heldPokemon[activePokemon].name, "");
            int swap = 999;
        ...
        if (enemy->hp == 0) {
            ...
            if (isWild) {
                mvwprintw(win, 5, 10, "Would you like to try capture it?%20s", "");
                ...
                while(strcmp(str, "") == 0) {
                    ...
                    if (strcmp(str, "1") == 0) {
                        if (numPokemon < 6 && pokeballs > 0) {
                            pokeballs--;
                            mvwprintw(win, 5, 10, "You threw a pokeball!%20s", "");
                            wrefresh(win);
                            usleep(1500000);
                            mvwprintw(win, 5, 10, "You captured %s!%-20s", enemy->name, "");
                            wrefresh(win);
                            ...
                            mvwprintw(win, 5, 10, "You already have 6 pokemon!");
                            mvwprintw(win, 6, 10, "The %s fled!", enemy->name);
                        }
                    } else if (strcmp(str, "2") == 0) {
                        continue;
                    } else {
...
void openingScreen() {
    ...
    for (int i = 0; i < 12; i++) {
        mvwprintw(win, i + 1, 4, art[i].c_str());
        wrefresh(win);
        usleep(50000);
    }

    usleep(1000000);
    mvwprintw(win, 15, 4, "...but not really");
    wrefresh(win);
    usleep(1000000);
    mvwprintw(win, 15, 60, " have fun!");
    wrefresh(win);
    usleep(2500000);

    for (int i = 0; i < 39; i++) {
        for (int j = 0; j < 17; j++) {
            mvwprintw(win, j + 1, i + 1, " ");
        }
        
        for (int j = 0; j < 17; j++) {
            mvwprintw(win, j + 1, 78 - i, " ");
        }
        wrefresh(win);
        usleep(25000);
    }
